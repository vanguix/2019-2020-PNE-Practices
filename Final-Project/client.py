...
def client(client_request, json_parameter):
    PORT = 8080
    SERVER = 'localhost'
    print(f"\nConnecting to server: {SERVER}:{PORT}\n")
    conn = http.client.HTTPConnection(SERVER, PORT)
    try:
        conn.request("GET", client_request + "&json=" + json_parameter)
...
while options:
    ...
    menu()
    json_parameter = input("Choose the json parameter(must be 1 for the server to work): ")
